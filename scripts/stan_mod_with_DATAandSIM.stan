...
transformed parameters { 
 ...
 // POPULATION PARAMETERS ESTIMATED WITHOUT COVARIATES 
 vector [nByrs] N_j_NC; // predicted juveniles 
 vector [nByrs] N_j_predicted_NC; // predicted juveniles this goes into the liklihood- gets transformed by estimates of Q
 vector [nByrs] N_e_sum_NC; // sum eggs across ages to then go into the lifecycle section that doesnt use age 
 vector [nByrs] N_brood_year_return_NC; // sum eggs across ages to then go into the lifecycle section that doesnt use age 
 real N_recruit_NC [nRyrs_T,A]; 
 real N_sp_NC [nRyrs_T,A];
 real N_catch_NC [nRyrs_T,A]; 
 real N_e_NC [nRyrs_T,A];
...
vector [nByrs] kappa_j_survival_NC; // predicted survival for juvenile fish (FW and early marine)
vector [nByrs] kappa_marine_survival_NC; // predicted survival for marine fish
...
for(t in 1:t_start){
  ...
  N_e[1:t_start,a] = N_egg_start[1:t_start,a]; 
     ...
     // add starting values for the _NC too
  N_j_NC[1] = N_j_start;
  N_brood_year_return_NC[1] = N_brood_year_return_start;
 
    for(a in 1:A){
   // add starting values to the whole population array
  N_recruit_NC[1:t_start,a] = N_recruit_start[1:t_start,a];
  N_sp_NC[1:t_start,a] = N_sp_start[1:t_start,a];
  N_catch_NC[1:t_start,a] = N_catch_start[1:t_start,a];
  N_e_NC[1:t_start,a] = N_egg_start[1:t_start,a]; 
     }
...
// Process model that contributes to the likelihood
...
// Process model that DOES NOT contribute to likelihood but looks at population trajectories without covariate influence
     for (t in 1:nByrs){ // loop for each brood year 
         N_e_sum_NC[t] = sum(N_e_NC[t,1:A]); // Eq. 4.11 
           
         kappa_j_survival_NC[t] =  (1 / (1 + exp(-basal_p_1)))/(1 + (((1 / (1 + exp(-basal_p_1)))*N_e_sum_NC[t])/c_1)); // Eq 4.2
         
         N_j_NC[t] = kappa_j_survival_NC[t]*N_e_sum_NC[t]; // Eq 4.1
     
         kappa_marine_survival_NC[t] =  (1 / (1 + exp(-basal_p_2)))/(1 + (((1 / (1 + exp(-basal_p_2)))*N_j_NC[t])/c_2)); 
         
         N_brood_year_return_NC[t] = N_j_NC[t]*kappa_marine_survival_NC[t]; // Eq 4.4          
        
        for (a in 1:A) { 
           N_recruit_NC[t+a+1,a] = (N_brood_year_return_NC[t]*p[t,a]) * exp(-(sum(M[1:a]))); //Eq 4.5 - part of it, see above for the rest of age comp estiamtion 

           // no selectivity 
           // N_catch[t+a+1,a] = N_recruit[t+a+1,a]*(1-exp(-(F[t+a+1]))); // Eq 4.7 
           // selectivity included 
           N_catch_NC[t+a+1,a] = N_recruit_NC[t+a+1,a]*(1-exp(-(F[t+a+1]*S[a]))); // Eq 4.7  
           
           N_sp_NC[t+a+1,a] = N_recruit_NC[t+a+1,a]-N_catch_NC[t+a+1,a];  // Eq 4.9
          
           N_e_NC[t+a+1,a] = (Ps*N_sp_NC[t+a+1,a]) * exp(alpha[a] - (beta * (Ps*N_sp_NC[t+a+1,a])));
         }
     }

for(t in 1:nByrs){
 // translate juvenile fish to the appropriate scale 
 N_j_predicted_NC[t]= catch_q*N_j_NC[t]; 
 }
