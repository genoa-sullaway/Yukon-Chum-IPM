...
model {
   sigma_y_j ~ uniform(0,2); //normal(0,8);
...
// Likelilihoods --  
  // Observation model
  for (t in 1:nByrs) {
     log(data_stage_j[t]) ~ normal(log(N_j_predicted[t]), sigma_y_j);// sqrt(log((0.15^2) + 1))); //sigma_y_j); // sqrt(log((data_j_cv[t]) + 1)));//sigma_y_j); //sqrt(log((0.08^2) + 1)));
    } 
