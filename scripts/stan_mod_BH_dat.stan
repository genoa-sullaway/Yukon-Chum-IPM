...
// real <lower=0.001, upper = 0.99>  p_1; // productivity in bev holt transition funciton, 1 = FW early marine
// real <lower=0.001, upper = 0.99>  p_2;
...
model {
  ...
  // 
  for(t in 1:nByrs){
    p_1[t] ~ beta(1,1); //normal(0,1);//beta(1,1);
    p_2[t] ~ beta(1,1);
  }
