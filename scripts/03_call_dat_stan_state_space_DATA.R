...
data_stage_j  <- as.matrix(juv$abund, nrow = nByrs, ncol = K) # juveniles from basis data, by brood year
...
for(k in 1:K) {  
  sigma_y_j[1,k] = rnorm(1,0.5)
  sigma_y_sp[1,k] = rnorm(1,0.5)
}

# capacity ==============================
c_1 <- as.matrix(nrow = 1, ncol =1, rnorm(1,1e8, 1e1)) 
c_2 <- as.matrix(nrow = 1, ncol =1, rnorm(1,1e6, 1e4)) 

# starting values for survival rate 
kappa_j_start =  matrix(data.frame(kappa=c(rep(runif(1, 0.2, 0.2), times =K)))) # ,
                # as.numeric(rep(runif(1, 0.03, 0.07), times = K)) #,
                # as.matrix(data.frame(kappa=c(rep(runif(1, 0.03, 0.07), times =K)))) # ,
                   # runif(1, 0.03, 0.07),
                   # runif(1, 0.03, 0.07))
kappa_marine_start = matrix(data.frame(kappa=c(rep(runif(1, 0.4, 0.4), times =K)))) # ,
...
# fix sigma covariates ===== 
set.seed(123)
sigma_coef1 <- matrix(ncol = K, nrow =ncovars1, NA)
sigma_coef2 <- matrix(ncol = K, nrow =ncovars2, NA)

for(k in 1:ncovars1) {  
  sigma_coef1[1,k] = (rnorm(1,1,1))
  sigma_coef2[1,k] = (rnorm(1,1,1))
}
sigma_coef1 <- matrix(sigma_coef1)
sigma_coef2 <- matrix(sigma_coef2)
...
cov2 =  as.matrix(stage_b_cov %>% 
                   ...
                   dplyr::mutate(yukon_mean_discharge_summer=as.numeric(scale(yukon_mean_discharge_summer))))
