...
theta2 <- c(-0.05)#, 0.1, 0.06, -0.06)#, -0.6) #relationship for simulated data
 ...
 # AGE STRUCTURE =========
  Dir_alpha = c(NA)
  p = c(NA) #matrix(nrow=nByrs+1,ncol=A,NA)
  g = c(NA)
  # g = matrix(nrow=nByrs+1,ncol=A,NA)
  D_scale = 0.2
...
# fix productivity ======
p_1 =  (rnorm(nByrs+1, 0.3,0.1)  ) 
p_2 =  (rnorm(nByrs+2, 0.5,0.1)  ) 
...
# POPULATION MODEL ============ 
     for (t in 1:nByrs){ # loop for each brood year 
         ...
         kappa_marine[t] =  p_2[t]/(1 + (( p_2[t]*N_j[t+1])/c_2)) # Eq 4.1  - Bev holt transition estimating survival from juvenile to spawner (plugs into Eq 4.4) 
         ...
         for (a in 1:A) { 
           ...
           #N_first_winter[t+a+1,a] =  N_j[t+1]*p[t+1,a]; #add age structure, p is proportion per age class
