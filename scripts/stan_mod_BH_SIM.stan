...
//starting values 
 ...
 // vector<lower=8> [t_start]N_sp_start_log;
 // vector<lower=8>[t_start]N_recruit_start_log;
...
model {
 ...
 for(t in 1:t_start){
  ...
  // print("N_e_sum_start_log:", N_e_sum_start_log);
    ...
    basal_p_1 ~ beta(0.3,1); // mean survival stage 1
    basal_p_2 ~ beta(0.4,1); // mean survivial stage 2
...
// Likelilihoods --  
 ...
 if(t<nByrs){
     ...
     target += normal_lpdf(log(data_stage_sp[t]) |  log(sum(N_sp[t,1:A])), sqrt(log((data_sp_cv[t]) + 1))); // sigma_y_sp);
      }
    }
  }
