...
real catch_q[K]; // related juvebile data to spawner data (on different scales) gets transfomed from log to number 
...
for(k in 1:K){  // loop for each population
  ...
  //add in age for stage where I am tracking age class.... 
   // swtich to calendar years using t+A-a
      for (a in 1:A) { 
     N_returning[t+A-a,k,a] = (catch_q[k]*N_recruit[t,k])*p[t+A-a,a]; // multiply recruits by Q to change scale, then distribute across age classes, p which is age comps. 
...
for(k in 1:K) { 
  //start off with sigma fixed
   ...
   log_catch_q[k] ~ normal(0,0.05); // Estimate Q - this will translate # of recruits to # of spawners 
}
...
// }
    ...
    c_1[1] ~  uniform(1e6, 1e8); // carrying capacity prior - stage 1

    c_2[1] ~  uniform(1e4, 1e6); // carrying capacity prior - stage 2
...
// Liklilihoods --  
  ...
  for(k in 1:K){
  for (t in 2:nByrs) {
    log(data_stage_j[t,k]) ~ normal(log(N_j[t,k]), sigma_y_j[1,k]);
    } 
  }
  for(k in 1:K){ // stocks 
