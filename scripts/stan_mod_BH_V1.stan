...
real data_log_stage_j[N];
real data_log_stage_sp[N];
...
real<lower=1> N_e[N]; // predicted eggs, basically a dummy step. 
real<lower=1> N_j[N]; // predicted juveniles - this goes into the liklihood, data involved 
real<lower=1> N_sp[N]; // predicted spawners - this goes into the liklihood, data involved 
real<lower=1> N_sp_start; 
...
model {
  // PRIORS
   sigma_y_j ~ cauchy(0,1);
   sigma_y_sp ~ cauchy(0,1); 
   // log_N_j ~ uniform(1,1e6); //8.0, 13.0); // log juvenile prior
   // log_N_sp ~ uniform(1,1e6); // log spawner prior 
   log_N_sp_start ~ normal(15,0.001); // 12-15
   
//for(k in 1:K){
  //avg_B1[k] ~ normal(10,10); // Stan specific loop to assign priors across stocks
   p_1 ~ uniform(0.0001,1.5); //10,10);
   p_2 ~ uniform(0.0001,1.5);//normal(0,1.5^2); //10,10);
   
   c_1 ~ uniform(1,1e6);// uniform(1000,100000); // normal(1,5); <- these are my old prior values, cc are the uniform values. 
   c_2 ~ uniform(1,1e6);// uniform(1000,100000); // normal(1,5);
//}
