...
for(k in 1:K){  // loop for each population
  for (i in 2:N){ //will need to add a loop in here for stocks too..
    ...
    N_r[i,k] = kappa_sp[i,k]*N_j[i,k]; // Eq 4.5 generated estiamte for the amount of fish each year and stock that survive to a spawning stage
   }
 }
}
...
model {
  ...
  for(k in 1:K){
   ...
   sigma_y_j[1] ~  normal(2.06,10);
   sigma_y_j[2] ~  normal(1.75,10);
   sigma_y_j[3] ~  normal(1.7,10);
