...
# load model ==============
bh_fit<- read_rds("output/stan_fit_with_DATAandSIM.RDS")
...
# Look at specific parameters ====
 ...
 # Plot Observed vs Predicted ========
  ## Spawners no covariates ==========
  pred_N_SP_NC <- summary(bh_fit, pars = c("N_sp_NC"), 
                       probs = c(0.1, 0.9))$summary %>%
    data.frame() %>%
    rownames_to_column()  %>%
...
## brood year return NO COVARIATE ====== 
pred_N_brood_year_return_NC <- summary(bh_fit, pars = c("N_brood_year_return_NC"),
                                    probs = c(0.1, 0.9))$summary %>%
  data.frame() %>%
  rownames_to_column()  %>%
...
ggplot(data = pred_N_brood_year_return_NC) +
  geom_line(aes(x=cal_year, y = mean)) +
  geom_point(aes(x=cal_year, y = obs)) +
  geom_ribbon(aes(x=cal_year, ymin = mean-se_mean,
                  ymax = mean+se_mean))+
  ggtitle(("brood year return NC"))
...
# harvest NC ======
pred_N_harvest_NC <- summary(bh_fit, pars = c("N_catch_NC"), 
                          probs = c(0.1, 0.9))$summary %>%
  data.frame() %>%
  rownames_to_column()  %>%
  dplyr::mutate(time = rep(1:26, each=4),
...
pred_N_j_NC <- summary(bh_fit, pars = c("N_j_NC"), 
                    probs = c(0.1, 0.9))$summary %>%
  data.frame() %>%
  rownames_to_column()  %>%
...
ggplot(data = summ_n_j_NC) +
  ...
  ggtitle(("Juveniles, est and observed NC"))
...
brood_year_j <- summ_n_j %>%
  dplyr::select(mean, obs, time) %>% 
  dplyr::rename(brood = "time",
         mean_juv = "mean",
         obs_j = "obs") %>% 
  dplyr::mutate(obs_j = obs_j*100,
         brood = brood-1) %>% 
...
# plot  estimated kappas survival NO COVAR ======
kappasurvival_NC <- summary(bh_fit, pars = c("kappa_marine_survival_NC", "kappa_j_survival_NC"), 
                         probs = c(0.1, 0.9))$summary %>%
  data.frame() %>%
  rownames_to_column()  %>% 
  dplyr::mutate(time = rep(1:21, length.out = nrow(.)), 
                variable = case_when(grepl("kappa_marine_survival_NC",rowname) ~ "kappa_marine_survival_NC",
                                     grepl("kappa_j_survival_NC",rowname) ~ "kappa_j_survival_NC")) %>% 
  left_join(years)# %>% 
#filter(!time<5 & !time>20)

ggplot(data = kappasurvival_NC,
       aes(x=cal_year, y = mean, group = variable ,color = variable)) + 
  geom_line( ) +
  geom_ribbon(aes(x=cal_year, ymin = mean-se_mean,
                  ymax = mean+se_mean), alpha = 0.5) + 
